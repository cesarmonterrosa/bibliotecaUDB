import java.awt.event.ActionEvent;
import java.awt.event.ActionListener;
import javax.swing.*;

public class SecurityQuestWindow extends JFrame {

    public SecurityQuestWindow() {
        // Set up the frame
        setTitle("Biblioteca UDB");
        setSize(400, 300);
        setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        setLocationRelativeTo(null); // Center the window on the screen

        // Create panel
        JPanel panel = new JPanel();
        panel.setLayout(null);
        add(panel);

        // Add components
        JLabel welcomeLabel = new JLabel("Responda la pregunta de seguridad: ");
        welcomeLabel.setBounds(90, 10, 250, 25);
        panel.add(welcomeLabel);
        JLabel questionLabel = new JLabel("###################");
        questionLabel.setBounds(90, 50, 250, 25);
        panel.add(questionLabel);

        JTextField textField = new JTextField(20); // 20 is the preferred width
        textField.setBounds(90, 90, 200, 25);
        panel.add(textField);

        JButton searchButton = new JButton("Continuar"); 
        searchButton.setBounds(120, 150, 150, 30);
        panel.add(searchButton);


        // Add action listeners for buttons
        searchButton.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                // Placeholder for Admin Portal functionality
                new NewPassWindow();
                dispose();
            }
        });

        // Make the frame visible
        setVisible(true);
    }



}
